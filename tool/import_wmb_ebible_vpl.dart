...
// World Messianic Bible — eBible.org VPL (public domain).
// Writes `assets/bible/wmb/` in the same shape as other bundled Bibles (Protestant 66).
//
//   dart run tool/import_wmb_ebible_vpl.dart
//
// Source: https://ebible.org/bible/details.php?id=engwmb
...
const _zipUrl = 'https://ebible.org/Scriptures/engwmb_vpl.zip';

/// Protestant canon order; book codes as in engwmb_vpl.txt (e.g. MAR, JOH, JAM, 1JO).
...
Future<File> _findVplTxt(Directory root) async {
  await for (final e in root.list(recursive: true, followLinks: false)) {
    if (e is File && e.path.endsWith('engwmb_vpl.txt')) return e;
  }
  throw StateError('engwmb_vpl.txt not found under ${root.path}');
}
...
Future<void> main() async {
  ...
  final tmp = await Directory.systemTemp.createTemp('wmb_ebible_');
  try {
    ...
    final zipFile = File('${tmp.path}/engwmb_vpl.zip');
    ...
    final outDir = Directory('${root.path}/assets/bible/wmb');
    await outDir.create(recursive: true);

    await File('${outDir.path}/LICENSE_ebible_engwmb.txt').writeAsString(
      'World Messianic Bible (engwmb).\n'
