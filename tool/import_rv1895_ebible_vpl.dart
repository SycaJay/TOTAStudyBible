...
// Revised Version with Apocrypha (1895), English — eBible.org VPL (public domain).
// Writes `assets/bible/rv1895/` in the same shape as other bundled Bibles (Protestant 66).
//
//   dart run tool/import_rv1895_ebible_vpl.dart
//
// Source: https://ebible.org/details.php?id=eng-rv
...
const _zipUrl = 'https://ebible.org/Scriptures/eng-rv_vpl.zip';

/// Protestant canon order; book codes as in eng-rv_vpl.txt (e.g. MAR, JOH, JAM, 1JO).
...
Future<File> _findVplTxt(Directory root) async {
  await for (final e in root.list(recursive: true, followLinks: false)) {
    if (e is File && e.path.endsWith('eng-rv_vpl.txt')) return e;
  }
  throw StateError('eng-rv_vpl.txt not found under ${root.path}');
}
...
Future<void> main() async {
  ...
  final tmp = await Directory.systemTemp.createTemp('rv1895_ebible_');
  try {
    ...
    final zipFile = File('${tmp.path}/eng-rv_vpl.zip');
    ...
    final outDir = Directory('${root.path}/assets/bible/rv1895');
    await outDir.create(recursive: true);

    await File('${outDir.path}/LICENSE_ebible_eng-rv.txt').writeAsString(
      'Revised Version with Apocrypha (1895), English (eng-rv).\n'
      'Imported from eBible.org VPL distribution (public domain).\n'
      'https://ebible.org/details.php?id=eng-rv\n',
    );
